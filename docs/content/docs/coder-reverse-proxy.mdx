...
For local development, we use a reverse proxy to access Coder workspaces at `coder.tryleopard.dev` through a local domain. This allows you to:

- Test workspace launching locally without modifying production
- Set cookies on `localhost` domain instead of `.tryleopard.dev`
...
1. A session token is set as a cookie on domain `.tryleopard.dev`
2. The user is redirected to `https://coder.tryleopard.dev/workspace/...`
...
- You can't set cookies for `.tryleopard.dev` from localhost
...
Redirect to http://coder.localhost/workspace/...
    ↓
Nginx Reverse Proxy (makes it look like HTTPS from coder.tryleopard.dev)
    ↓
https://coder.tryleopard.dev (actual Coder instance)
```

**Architecture Components:**

1. **Docker container** runs nginx on port 80
2. **nginx** forwards all requests to `https://coder.tryleopard.dev`
...
proxy_set_header Origin https://coder.tryleopard.dev;
```
...
- `Origin https://coder.tryleopard.dev` makes Coder accept the request as legitimate
...
1. **Origin header not set correctly**
   - Check nginx.conf has `proxy_set_header Origin https://coder.tryleopard.dev;`
...
- Direct connection to `https://coder.tryleopard.dev`
- Cookies with `secure: true` (HTTPS only)
- Domain set to `.tryleopard.dev`
