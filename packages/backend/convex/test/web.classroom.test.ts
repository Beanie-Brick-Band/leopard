...
import { seedClassroom, seedEnrollment, seedUserRole } from "./testHelpers";
...
function makeTestClient() {
  const modules: Record<string, () => Promise<unknown>> = {
    ...
    "convex/web/classroom.ts": () => import("../web/classroom"),
  };
...
describe("web/classroom", () => {
  describe("getEnrolled", () => {
    // Ensures unauthenticated users get an empty list instead of an error.
    ...
    // Ensures only students can use enrollment list endpoints.
    it("denies teacher/admin", async () => {
      const t = makeTestClient();
    ...
    // Ensures the query returns only classrooms the student is enrolled in.
    it("returns enrolled classrooms", async () => {
      ...
      const res = await t.query(api.web.classroom.getEnrolled, {});
      const ids = res.filter(Boolean).map((c: any) => c._id);
  ...
  describe("getAvailableToEnroll", () => {
    // Ensures unauthenticated users get an empty list.
    it("returns [] when not logged in", async () => {
      const t = makeTestClient();
      safeGetAuthUser.mockResolvedValue(null);
    ...
    // Ensures only students can query available classrooms.
    it("denies teacher/admin", async () => {
      const t = makeTestClient();
    ...
    // Ensures the query excludes classrooms the student is already enrolled in.
    it("returns classrooms the student is not enrolled in", async () => {
      ...
      const res = await t.query(api.web.classroom.getAvailableToEnroll, {});
      const ids = res.map((c: any) => c._id).sort();
  ...
  describe("enroll", () => {
    // Ensures enrollment requires authentication.
    ...
    // Ensures only students can enroll.
    it("denies teacher/admin", async () => {
      const t = makeTestClient();
      const classroomId = await seedTestClassroom(t, { className: "C1" });
      await seedUserRole(t, "teacher_1", "teacher");
    ...
    // Ensures enroll() creates the student->classroom relation.
    ...
    // Ensures enroll() prevents duplicate enrollment relations.
