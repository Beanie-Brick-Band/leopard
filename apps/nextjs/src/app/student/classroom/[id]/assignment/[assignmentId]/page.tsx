...
function Content({
  ...
  const submissionResult = useQuery(api.web.submission.getOwnSubmissionsForAssignment, {
    assignmentId,
  });
  ...
  const grade = submission && "grade" in submission ? submission.grade : undefined;
  const feedback =
    submission && "submissionFeedback" in submission
      ? submission.submissionFeedback
      : undefined;
  const gradedAt = submission && "gradedAt" in submission ? submission.gradedAt : undefined;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(hasSubmission ? "Assignment re-submitted." : "Assignment submitted.");
    } catch (error) {
          ...
          {grade !== undefined ? (
            <Card className="border-green-500/50">
              <CardHeader>
                ...
                <CardDescription>
                  {gradedAt ? `Graded on ${new Date(gradedAt).toLocaleString()}` : null}
                </CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardContent className="py-6 text-sm text-muted-foreground">
                Submitted{" "}
                {submittedAt ? `on ${new Date(submittedAt).toLocaleString()}.` : "successfully."}{" "}
                Grading is pending.
              </CardContent>
            </Card>
          ) : null}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                This action is irreversible. You will not be able to submit again or relaunch
                this workspace after submission.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex justify-end gap-2">
              <Button variant="outline" onClick={closeSubmitConfirm} disabled={isSubmitting}>
                Cancel
              </Button>
