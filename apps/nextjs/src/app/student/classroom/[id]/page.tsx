      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" asChild>
            <Link href={`/student/classroom/${classroomId}/assignment/${assignment._id}`}>
              View Details
            </Link>
          </Button>
        </div>
...
function Content({ classroomId }: { classroomId: Id<"classrooms"> }) {
  ...
  const enrolledClassroom = enrolled?.find((classroom) => classroom?._id === classroomId);
  const availableClassroom = available?.find((classroom) => classroom._id === classroomId);
  const classroom = (enrolledClassroom ?? availableClassroom ?? null) as Classroom | null;
